...
function RoofVisual({ panelsCount, kwp }: RoofVisualProps) {
  ...
  return (
    <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 mb-6">
      <div className="flex items-center justify-between text-xs font-mono text-slate-600 mb-2.5">
        <span>Arranjo Visual Estimado no Telhado</span>
        <span className="text-amber-600 font-bold">{kwp} kWp ({panelsCount} módulos)</span>
      </div>

      <div className="grid grid-cols-6 sm:grid-cols-9 gap-1.5 p-3 rounded-xl bg-white border border-slate-200 shadow-inner">
        {[...Array(displayPanels)].map((_, i) => (
          <div
            key={i}
            className="h-8 rounded-sm bg-gradient-to-br from-sky-950 to-slate-900 border border-sky-400/40 flex items-center justify-center shadow-sm"
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
      <div className="p-4 rounded-2xl bg-rose-50 border border-rose-200 shadow-sm">
        <div className="flex items-center gap-1.5 text-xs font-mono text-rose-700 mb-1">
          <ShieldAlert className="w-3.5 h-3.5 text-rose-600" /> Sem Energia Solar (Neoenergia)
        </div>
        <div className="text-2xl font-mono font-bold text-slate-700 line-through opacity-70">
          {formattedBill}
        </div>
        <p className="text-[11px] text-slate-500 mt-1 font-normal">Custo mensal perdido com tarifas e bandeiras</p>
      </div>

      <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-200 shadow-sm">
        <div className="flex items-center gap-1.5 text-xs font-mono text-emerald-700 mb-1">
          <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" /> Com Usina DMJ Solar
        </div>
        <div className="text-2xl font-mono font-bold text-emerald-700">
          {formattedNew} <span className="text-xs font-normal text-slate-600">taxa mínima</span>
        </div>
        <p className="text-[11px] text-emerald-700 mt-1 font-mono font-semibold">Economia livre de {formattedSave}/mês</p>
      </div>
    </div>
  );
}
...
function ArchitectControls({
  ...
  return (
    <div className="lg:col-span-6 rounded-3xl bg-white border border-slate-200/90 p-6 sm:p-8 shadow-sm space-y-6">
      <div>
        <div className="flex justify-between items-center mb-2">
          <span className="text-xs font-mono uppercase tracking-widest text-slate-500">Gasto Mensal de Energia</span>
          <span className="text-2xl sm:text-3xl font-mono font-bold text-amber-600">{formattedBill}</span>
        </div>
        <input
          type="range"
          min={300}
      ...
      <button
        onClick={onAiRequest}
        disabled={loadingAi}
        className="w-full flex items-center justify-center gap-2 py-3.5 rounded-xl text-xs font-mono font-semibold text-amber-800 bg-amber-50 hover:bg-amber-100 border border-amber-300 transition-all active:scale-95 disabled:opacity-50 shadow-sm"
      >
        {loadingAi ? (
          <>
      ...
      {aiInsight && (
        <div className="p-4 rounded-xl bg-amber-50/80 border border-amber-200 text-xs space-y-1.5 text-slate-700 shadow-sm">
          <div className="font-mono font-bold text-amber-700 flex items-center gap-1.5">
            <Sparkles className="w-3.5 h-3.5" /> Análise de Viabilidade
          </div>
          <p className="leading-relaxed font-normal">{aiInsight.analysis}</p>
          <p className="text-[11px] font-mono text-amber-800 font-semibold">⚙️ {aiInsight.recommendation}</p>
        </div>
      )}
    </div>
  );
    ...
    <div className="lg:col-span-6 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="p-5 rounded-2xl bg-white border border-emerald-200 shadow-sm">
          <div className="flex items-center gap-1.5 text-xs font-mono text-slate-500">
            <TrendingUp className="w-4 h-4 text-emerald-600" /> Economia em 1 Ano
          </div>
          <div className="text-2xl sm:text-3xl font-mono font-bold text-emerald-700 mt-2">
            {formattedAnnual}
          </div>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-sm">
          <div className="flex items-center gap-1.5 text-xs font-mono text-slate-500">
            <Zap className="w-4 h-4 text-amber-600" /> Economia em 25 Anos
          </div>
          <div className="text-2xl sm:text-3xl font-mono font-bold text-slate-900 mt-2">
            {formatted25Y}
          </div>
        </div>
      </div>
      ...
      <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 flex items-center justify-between text-xs font-mono">
        <span className="text-slate-600 flex items-center gap-1.5">
          <Clock className="w-4 h-4 text-emerald-600" /> Payback Médio Estimado
        </span>
        <span className="text-emerald-700 font-bold">~{paybackYears} Anos</span>
      </div>

      <div className="p-6 rounded-3xl bg-gradient-to-r from-amber-50/60 via-white to-emerald-50/60 border border-amber-300 shadow-md space-y-4">
        <div>
...
export function SolarArchitect() {
  ...
  return (
    <section id="simulador" className="py-20 sm:py-28 bg-[#F8FAFC] relative border-t border-slate-200/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="mb-8 p-4 rounded-2xl bg-gradient-to-r from-emerald-50 via-white to-emerald-50 border border-emerald-200/90 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs sm:text-sm text-slate-700">
                Fatura de <strong className="text-rose-600 line-through">R$ 567,48</strong> caiu para taxa mínima de{" "}
                <strong className="text-emerald-700 font-mono font-bold">R$ 28,37</strong> (95% de corte líquido)
              </p>
            </div>
          </div>
          <span className="text-xs font-mono font-bold bg-emerald-600 text-white px-3.5 py-1 rounded-full whitespace-nowrap shadow-sm">
            ECONOMIA DE 95%
          </span>
        </div>
